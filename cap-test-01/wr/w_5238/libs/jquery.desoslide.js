...
(function ($, window, document, undefined) {
    ...
    defaults = {
        ...
        interval:           1000,             // Interval between each images
        effect: {
    ...
    Plugin.prototype = {
                ...
                if ($(item).attr('href') !== undefined) {
                    ...
                    if ($(item).find('img').length) {
                        ...
                        self.props.thumbs.push({
							...
							name:           $(item).find('img').attr('name') || null,
        ...
        */
        _wrapper: function () {
			var $img = $('<iframe>').attr('name', this._name).attr('id', "carouselMainImage").addClass(this.options.imageClass);
			$(this.elem).html($img).wrapInner($('<div>', {
				'class': this._namespace +'-wrapper'
			}));
			
			this.props.img.$elem = $(this.elem).find('iframe:first');
			//$($(this.props.img.$elem).contents().find("body")).addClass('overflow-hidden');
        ...
        */
        _showImage: function () {
			...
			currentAssetType = this.props.thumbs[this.props.img.to_show].name
			checkAutoPlayTimer()
            ...
            this.props.img.$elem
				...
				.attr('name', this.props.thumbs[this.props.img.to_show].name)
				.attr('frameBorder', 0)
				.css('overflow', 'hidden')
                ...
                .on('load', function () {
					if($(this).attr("name") == "image"){
						var imageInIframe = $($(this).contents().find("img"));
						$(imageInIframe).css('width',AspectRatioWidth);
						$(imageInIframe).css('height',AspectRatioHeight);
						$(imageInIframe).css('overflow','hidden');
					}
					$('#carouselContent').show();
					$("#carouselMainImage").contents().get(0).getElementsByTagName("body")[0].style.overflow = "hidden";
                    ...
                    if (self.options.auto.start === true) {
						self.props.img.timer = setTimeout(function () {
							self.goNext(true);
						}, (self.options.interval < 1000) ? 1000 : self.options.interval);
                    }
                });
        },
